...
#with open("Supp_File_S5_CAGE_Dpse_F_carcass.bed") as fileDpse, open("Orthologous_sequences_from_Dpse.bed","w") as Dpse_outfile:
...
with open("Supp_File_S1_CAGE_Dmel_FM_carcass.bed") as fileDpse, open("Orthologous_sequences_from_Dmel.bed","w") as Dpse_outfile:
    for line in fileDpse:
        record = line.strip().split()
            # for index in range(len(ortho_sequences)):
        if record[3] in ortho_sequences:
...
with open("Orthologous_sequences_from_Dmel.bed") as orig_file, open("ortholog_sequences_Dmel.bed",'w') as new_fi:
...
call("./twoBitToFa dm3.2bit dmel_output_ortho.fa -bed=ortholog_sequences_Dmel.bed", shell=True)
...
replace_lines = open("Orthologous_sequences_from_Dmel.bed")
########### Reverse Complement ############################
alt_map = {'ins':'0'}
...
f_file = open("dmel_output_ortho.fa")  ## The input FASTA file. This does not contain strand information and reverse complement hasn't been done
out_file = open("All_orthologs_fasta_Dmel.fa","w")  ## The required output FASTA file. This will be used in NPLB
...
with open("image_code_matrix_Dmel_Orthologs.csv",'w') as matrix_file:
